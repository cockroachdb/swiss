...
func makeCtrlGroup(ctrls []ctrl) *ctrlGroup {
	var g ctrlGroup
	for i, c := range ctrls {
		g.Set(uint32(i), c)
	}
...
func TestMatchH2(t *testing.T) {
	...
	for i := uintptr(1); i <= 8; i++ {
		match := makeCtrlGroup(ctrls).matchH2(i)
		bit := match.first()
...
func TestMatchEmpty(t *testing.T) {
	...
	for _, c := range testCases {
		t.Run("", func(t *testing.T) {
			match := makeCtrlGroup(c.ctrls).matchEmpty()
			var results []uint32
			for match != 0 {
				idx := match.first()
...
func TestMatchEmptyOrDeleted(t *testing.T) {
	...
	for _, c := range testCases {
		t.Run("", func(t *testing.T) {
			match := makeCtrlGroup(c.ctrls).matchEmptyOrDeleted()
			var results []uint32
			for match != 0 {
				idx := match.first()
				results = append(results, idx)
...
func TestConvertNonFullToEmptyAndFullToDeleted(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		makeCtrlGroup(ctrls).convertNonFullToEmptyAndFullToDeleted()
...
func TestIterateDelete(t *testing.T) {
	...
	// Iterate over the map, deleting elements periodically. We should see all of
	// the elements that were originally in the map because All takes a
	// snapshot of the ctrls and slots before iterating.
	vals := make(map[int]int)
	m.All(func(k, v int) bool {
...
func TestIterateTerminatesEarly(t *testing.T) {
	m := New[int, int](0)
	m.Put(1, 1)
	m.Put(2, 2)
	m.Put(3, 3)
	...
	m.All(func(key int, value int) bool {
		count++
		if count == 2 {
			return false // Terminate iteration after 2 elements
		}
		return true
	})
